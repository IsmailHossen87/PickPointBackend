...
const sslPaymentInit =async(payload : ISSLCommerz)=>{
   try {
     const data = {
            ...
            success_url: `${envVars.SSL.SSL_SUCESS_BACKEND_URL}?transactionId=${payload.transactionId}&amount=${payload.amount}&status=success`,
            fail_url: `${envVars.SSL.SSL_FAIL_BACKEND_URL}?transactionId=${payload.transactionId}&amount=${payload.amount}&status=fail`,
            cancel_url: `${envVars.SSL.SSL_CANCEL_BACKEND_URL}?transactionId=${payload.transactionId}&amount=${payload.amount}&status=cancel`,
             // ipn_url: "http://localhost:3030/ipn",     last e niser ta korte hobe
...
const validatePayment = async(payload:any) =>{
   try {
    ...
    console.log("ssl commer validateAPI response",response.data)
